export default ({ cards }) => (
    <div className='col-sm-row-md space-evenly wrap'>
        { cards.map(({ price, description, example_url }) =>
            <div className='col halign site-pricing-card'>
                ...
                <div className='col halign'>
                    ...
                    { example_url && <button onClick={ () => window.location.href = example_url }
                        className='site-pricing-card-body-button'>Example</button>}
                    { (!(example_url)) && <button disabled
                        className='site-pricing-card-body-button'>Coming Soon</button>}
                </div>
            </div>
        )}
    </div>
